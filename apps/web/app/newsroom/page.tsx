...
export default async function Newsroom() {
  const state = F.STATE
  const [{ data, session, source }, others, headlines] = await Promise.all([
    getNewsroom(state),
    getStream({
      states: DESKS.filter((d) => d !== state).slice(0, 3),
      limit: 2,
    }),
    getStories(state, { limit: 8 }),
  ])
  if (!data)
    return (
      <p className="container-wrapper px-6 py-10 text-sm text-muted-foreground">
        Nothing on file for this desk.
      </p>
    )
  return (
    <div data-source={source}>
      <NewsroomScoped
        ...
        initialOthers={
          others.source === "database"
            ? others.groups
            : (desks as unknown as StreamGroup[])
        }
