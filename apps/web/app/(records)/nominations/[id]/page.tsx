      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="mx-auto flex w-full max-w-160 min-w-0 flex-1 flex-col gap-6 px-4 py-6 text-foreground md:px-0 lg:py-8 dark:text-foreground">
          <RecordHeader
            ...
            action={
              <>
                <DocsCopyPage page={markdown} url={`https://gov.nysgpt.com/nominations/${encodeURIComponent(name.toLowerCase())}`} />
                {neighbours.previous ? (
                  <Button variant="secondary" size="icon" className={arrow} asChild>
                    <Link href={`/nominations/${encodeURIComponent((neighbours.previous.citation ?? neighbours.previous.key).toLowerCase())}`} title={neighbours.previous.citation ?? undefined}>
                      <IconArrowLeft />
                ...
                {neighbours.next ? (
                  <Button variant="secondary" size="icon" className={arrow} asChild>
                    <Link href={`/nominations/${encodeURIComponent((neighbours.next.citation ?? neighbours.next.key).toLowerCase())}`} title={neighbours.next.citation ?? undefined}>
                      <IconArrowRight />
            ...
            <div className="hidden h-16 w-full items-center gap-2 px-4 sm:flex sm:px-0">
              {neighbours.previous && (
                <Button variant="secondary" size="sm" className="shadow-none" asChild>
                  <Link href={`/nominations/${encodeURIComponent((neighbours.previous.citation ?? neighbours.previous.key).toLowerCase())}`}>
              ...
              {neighbours.next && (
                <Button variant="secondary" size="sm" className="ml-auto shadow-none" asChild>
                  <Link href={`/nominations/${encodeURIComponent((neighbours.next.citation ?? neighbours.next.key).toLowerCase())}`}>
