...
const typesetSearchParams = {
  ...
  // The legislative rail (same keys as /create's) plus the bill's own
  // version and related-bill picks. `mode` switches the rail between the
  // bill filters and the type pickers.
  mode: parseAsStringLiteral(["state", "design"] as const).withDefault("state"),
  // Empty, not New York: the jurisdiction hook resolves the scope (URL →
  // this browser's last choice → New York) so the header switcher reaches
  // /typeset too.
  state: parseAsString.withDefault(""),
  session: parseAsString.withDefault(""),
  chamber: parseAsString.withDefault(""),
  committee: parseAsString.withDefault(""),
  member: parseAsString.withDefault(""),
  party: parseAsString.withDefault(""),
  status: parseAsString.withDefault(""),
  subject: parseAsString.withDefault(""),
  vote: parseAsString.withDefault(""),
  bill: parseAsString.withDefault(""),
  version: parseAsString.withDefault(""),
  related: parseAsString.withDefault(""),
  chat: parseAsString.withDefault(""),
}

// The keys whose change means the preview iframe must re-render its content
// (the bill changed), as opposed to restyling in place.
export const TYPESET_CONTENT_KEYS = [
  "item",
  "state",
  "session",
  "chamber",
...
// The legislative keys are free-form strings and pass through.
export function coerceTypesetValue<K extends keyof TypesetSearchParams>(
  key: K,
  value: string
) {
  const allowed = (
    TYPESET_PARAM_VALUES as Record<string, readonly string[] | undefined>
  )[key]
  if (!allowed) {
    return value as TypesetSearchParams[K]
  }
  return allowed.includes(value) ? (value as TypesetSearchParams[K]) : null
}
