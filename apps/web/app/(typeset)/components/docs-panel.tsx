...
import {
  ...
  TabsList,
  TabsTrigger,
} from "@govblock/ui/components/nova/tabs"
import {
  TEMPLATES,
  type TemplateValue,
} from "@/app/(typeset)/lib/templates"
import { findFont, findFontDefinition } from "@/app/(typeset)/lib/fonts"
import {
  TYPESET_MEASURES,
  useTypesetSearchParams,
} from "@/app/(typeset)/lib/search-params"

// The panel fills the height of the main container beside it (v3 capped it
// at calc(100vh-16rem)); Brendan, 2026-09-01.
export function TypesetDocsPanel() {
  return (
    <div className="hidden min-h-0 w-88 flex-col items-start gap-3 self-stretch xl:flex 2xl:w-104">
      <div className="isolate flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-2xl bg-background ring-1 ring-foreground/10">
        <TypesetDocsContent />
      </div>
      <Button
        variant="link"
...
export function TypesetDocsContent() {
  ...
  const [framework] = React.useState<TemplateValue>("next")
  ...
  return (
    <Tabs defaultValue="chat" className="flex min-h-0 flex-1 flex-col gap-0">
      ...
      <div className="min-h-0 flex-1 scroll-fade scrollbar-none overflow-y-auto p-4 md:p-6">
        ...
        <TabsContent value="docs" className="hidden flex-col gap-6">
...
type ChatInstance = { id: string; title: string; created: string }
const NEW_CHAT = "__new__"

// The dropdown holds the chat instances: one per bill by default, more when
// you start them. The current one rides the `chat` search param.
function ChatPicker() {
  ...
  const [chats, setChats] = useLocal<ChatInstance[]>("livingston:chats", [])
    ...
    <Select
      ...
      onValueChange={(next) => {
        if (next === NEW_CHAT) {
          ...
          void setParams({ chat: id })
          return
        }
        void setParams({ chat: next === defaultId ? "" : String(next ?? "") })
      }}
    >
...
// The assistant beside the page, framed by the bill in the rail.
function AssistPanel() {
  ...
  const { data: bill } = usePolicy<Bill>("bill", filters)
  const chatId = params.chat || (bill ? `bill-${bill.bill_id}` : "workspace")
...
// The framework picker the Prompt tab used to show; kept for the install
// instructions, not mounted.
