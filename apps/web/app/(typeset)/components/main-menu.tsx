...
import { Menu09Icon, Tick02Icon } from "@hugeicons/core-free-icons"
import { HugeiconsIcon } from "@hugeicons/react"

import { cn } from "@/lib/utils"
import { type Button } from "@govblock/ui/components/nova/button"
import {
  Picker,
  PickerContent,
  PickerGroup,
  ...
  PickerShortcut,
  PickerTrigger,
} from "@/app/(typeset)/components/picker"
...
export function TypesetMainMenu({
  ...
  const mode = params.mode === "design" ? "Design" : "State"
  ...
  return (
    <Picker>
      ...
      <PickerContent side="right" align="start" alignOffset={-8}>
        <PickerGroup>
          <PickerItem onClick={() => setParams({ mode: "state" })}>
            State
            {mode === "State" && (
              <HugeiconsIcon icon={Tick02Icon} strokeWidth={2} className="ml-auto size-4" />
            )}
          </PickerItem>
          <PickerItem onClick={() => setParams({ mode: "design" })}>
            Design
            {mode === "Design" && (
              <HugeiconsIcon icon={Tick02Icon} strokeWidth={2} className="ml-auto size-4" />
            )}
