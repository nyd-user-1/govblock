...
import {
  ...
  type TypesetCommand,
} from "@/app/(typeset)/components/forward-scripts"
import { usePreviewOverrideValue } from "@/app/(typeset)/components/preview-override"
import { TypesetToolbar } from "@/app/(typeset)/components/toolbar"
import { useHistory } from "@/app/(typeset)/hooks/use-history"
import { useShuffle } from "@/app/(typeset)/hooks/use-shuffle"
import { useThemeToggle } from "@/app/(typeset)/hooks/use-theme-toggle"
import {
  serializeTypesetSearchParams,
  TYPESET_CONTENT_KEYS,
...
export function TypesetPreview() {
  ...
  // The content key: the page plus the bill filters. A change here reloads
  // the iframe (new content); anything else restyles it in place.
  const contentKeyOf = (p: TypesetSearchParams) =>
    TYPESET_CONTENT_KEYS.map((key) => String(p[key] ?? "")).join("|")
  const itemRef = React.useRef(contentKeyOf(params))
  ...
  React.useEffect(() => {
    ...
    const contentKey = contentKeyOf(mergedParams)
    if (contentKey !== itemRef.current) {
      itemRef.current = contentKey
      setPreviewUrl(
        serializeTypesetSearchParams(
