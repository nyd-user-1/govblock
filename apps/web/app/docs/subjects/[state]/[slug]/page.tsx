...
// One subject's bills — our version of congress.gov's search for a subject
...
// in a drawer.
//
// The path names the jurisdiction and the term — /docs/subjects/us/health —
...
export default async function SubjectRoute({ params }: Props) {
  ...
  const href = (name: string) => `/docs/subjects/${data.state.toLowerCase()}/${subjectSlug(name)}?state=${data.state}`
