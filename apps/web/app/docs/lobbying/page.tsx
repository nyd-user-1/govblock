...
const description =
  "Who is paid to be heard in Washington, on whose behalf, on what, and what it was reported to be worth."
...
const href = (kind: string, name: string) =>
  `/docs/lobbying/${kind}s/${encodeURIComponent(name)}`

type Props = { searchParams: Promise<{ q?: string }> }

export default async function LobbyingPage({ searchParams }: Props) {
  const term = String((await searchParams).q ?? "").trim()
  const [{ totals, firms, clients, lobbyists, sectors }, hits] =
    await Promise.all([
      getLobbyingOverview().catch(() => ({
        totals: null,
        firms: [],
        clients: [],
        lobbyists: [],
  ...
  return (
    ...
    >
      <p>
        Every firm that lobbies Congress files quarterly under the Lobbying
        Disclosure Act, naming the client, the issues, the lobbyists and the
        bills. The register holds <code>{fmtNumber(totals?.filings ?? 0)}</code>{" "}
        filings from <code>{fmtNumber(totals?.registrants ?? 0)}</code>{" "}
        registrants for <code>{fmtNumber(totals?.clients ?? 0)}</code> clients,
        naming <code>{fmtNumber(totals?.lobbyists ?? 0)}</code> lobbyists and{" "}
        <code>{fmtNumber(totals?.bills ?? 0)}</code> bills
        {totals?.first_year
          ? `, ${totals.first_year} to ${totals.last_year}`
          : ""}
        .
      </p>
      <p>
        A filing&rsquo;s income is what one registrant reported from one client
        for everything they worked that quarter. The LDA asks for no breakdown
        by issue or by bill, so no figure here is divided between them: a total
        under a firm is the sum of its filings, not a price for any one thing it
        did.
      </p>
      ...
      {term && (
        <>
          <p>
            {hits.length ? (
              <>
                <code>{fmtNumber(hits.length)}</code>{" "}
                {hits.length === 1 ? "match" : "matches"} for &ldquo;{term}
                &rdquo;.
              </>
            ) : (
      ...
      <p>By reported income across every filing they have made.</p>
          ...
          <tbody>
            {firms.map((row) => (
              <tr key={row.registrant}>
                <td>
                  <Link href={href("firm", row.registrant)}>
                    {row.registrant}
                  </Link>
                </td>
                <td className="text-right tabular-nums">
                  {fmtNumber(row.clients)}
                </td>
                <td className="text-right tabular-nums">
        ...
        <Table>
          ...
          <tbody>
            {lobbyists.map((row) => (
              <tr key={row.lobbyist}>
                <td>
                  <Link href={href("lobbyist", row.lobbyist)}>
                    {row.lobbyist}
                  </Link>
                </td>
                <td className="text-right tabular-nums">
      ...
      <p>
        The LDA&rsquo;s own issue codes, which is the only sector a filing
        declares: <code>{sectors.length}</code> of them in use.
      </p>
