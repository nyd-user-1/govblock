...
const COUNTERPARTY: Record<
  EntityKind,
  { heading: string; column: string; sentence: string }
> = {
  ...
  lobbyist: {
    heading: "Firms",
    column: "Registrant",
    sentence: "they have filed under",
  },
}
...
export default async function LobbyingEntityPage({ params }: Props) {
  ...
  const years =
    entity.first_year === entity.last_year
      ? String(entity.last_year ?? "")
      : `${entity.first_year} to ${entity.last_year}`
  const otherHref = (name: string) =>
    `/docs/lobbying/${entity.kind === "firm" ? "clients" : "firms"}/${encodeURIComponent(name)}`
  ...
  return (
    ...
    >
      <p>
        <Chip>{entity.name}</Chip> is{" "}
        {entity.kind === "lobbyist"
          ? "a registered lobbyist named on"
          : entity.kind === "firm"
            ? "a registrant on"
            : "a client on"}{" "}
        <code>{fmtNumber(entity.filings)}</code> federal lobbying{" "}
        {entity.filings === 1 ? "filing" : "filings"}
        {years ? `, ${years}` : ""}
        {entity.income != null ? (
          <>
      ...
      <p>
        An LDA filing reports one registrant&rsquo;s income from one client for
        everything they worked that quarter, so a total here is the sum of whole
        filings and never a price for one issue or one bill.
      </p>

      <hr />
      <H2 id="counterparties">{words.heading}</H2>
      <p>
        <code>{fmtNumber(entity.counterparties.length)}</code> {words.sentence},
        by filings.
      </p>
      ...
      {entity.bills.length > 0 && (
        <>
          ...
          <p>By how many filings name them.</p>
              ...
              <tbody>
                {entity.lobbyists.map((row) => (
                  <tr key={row.lobbyist}>
                    <td>
                      <Link
                        href={`/docs/lobbying/lobbyists/${encodeURIComponent(row.lobbyist)}`}
                      >
                        {row.lobbyist}
                      </Link>
                    </td>
