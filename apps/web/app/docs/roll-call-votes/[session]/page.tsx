...
export default async function RollCallSessionPage({
  params,
  searchParams,
}: Props) {
  const scope = parseSessionSlug((await params).session)
  ...
  const at = sessions.findIndex(
    (s) => sessionSlug(s.chamber, s.congress, s.session) === slug
  )
  ...
  return (
    <DocsPage
      ...
      slug={`/docs/roll-call-votes/${slug}`}
      previous={
        previous
          ? {
              name: `${chamberName(previous.chamber)} ${sessionName(previous.session)}`,
              url: `/docs/roll-call-votes/${sessionSlug(previous.chamber, previous.congress, previous.session)}`,
            }
          : { name: "Roll call votes", url: "/docs/roll-call-votes" }
      }
      next={
        next
          ? {
              name: `${chamberName(next.chamber)} ${sessionName(next.session)}`,
              url: `/docs/roll-call-votes/${sessionSlug(next.chamber, next.congress, next.session)}`,
            }
          : { name: "Roll call votes", url: "/docs/roll-call-votes" }
      }
    >
      <p>
        Roll call <code>{rows[rows.length - 1]?.roll ?? 0}</code> to{" "}
        <code>{rows[0]?.roll ?? 0}</code> of <code>{fmtNumber(total)}</code>,{" "}
        {onBills} of them on a bill or resolution and the rest on nominations,
        motions and procedure. A roll number opens the vote and every
        member&rsquo;s position on it.
      </p>
            ...
            {rows.map((r) => (
              <tr key={r.roll}>
                <td className="tabular-nums">
                  <Link href={`/docs/roll-call-votes/${slug}/${r.roll}`}>
                    {r.roll}
                  </Link>
                </td>
                <td className="whitespace-nowrap">
                  {r.date ? fmtDate(r.date.slice(0, 10)) : "—"}
                </td>
                <td className="whitespace-nowrap">
                  {r.citation ? (
                    r.bill_id ? (
                      <Link href={`/docs/bills/${r.bill_id}`}>
                        {r.citation}
                      </Link>
                    ) : (
                      r.citation
                ...
                <td>
                  {truncate(r.question ?? r.description ?? "", 120) || "—"}
                </td>
                <td>{r.result ?? "—"}</td>
      ...
      {pages > 1 && (
        <p>
          ...
          {page > 1 && (
            <>
              <Link href={`/docs/roll-call-votes/${slug}?page=${page - 1}`}>
                Newer
              </Link>
              {page < pages ? " · " : ""}
            </>
      ...
      <p>
        ...
        {scope.chamber === "house" ? (
          ...
          <a
            href={`https://www.senate.gov/legislative/LIS/roll_call_lists/vote_menu_${scope.congress}_${scope.session}.htm`}
            target="_blank"
            rel="noopener noreferrer"
          >
            senate.gov
