...
export default async function RollCallSessionPage({ params, searchParams }: Props) {
  ...
  return (
    <DocsPage
      ...
      previous={previous ? { name: `${chamberName(previous.chamber)} ${sessionName(previous.session)}`, url: `/docs/roll-call-votes/${sessionSlug(previous.chamber, previous.congress, previous.session)}` } : { name: "Roll call votes", url: "/docs/roll-call-votes" }}
      next={next ? { name: `${chamberName(next.chamber)} ${sessionName(next.session)}`, url: `/docs/roll-call-votes/${sessionSlug(next.chamber, next.congress, next.session)}` } : { name: "Roll call votes", url: "/docs/roll-call-votes" }}
    >
      <p>
          ...
          {rows.map((r) => (
            <tr key={r.roll}>
              <td className="tabular-nums">
                <Link href={`/docs/roll-call-votes/${slug}/${r.roll}`}>{r.roll}</Link>
              </td>
              <td className="whitespace-nowrap">{r.date ? fmtDate(r.date.slice(0, 10)) : "—"}</td>
              <td className="whitespace-nowrap">
                {r.citation ? r.bill_id ? <Link href={`/docs/bills/${r.bill_id}`}>{r.citation}</Link> : r.citation : "—"}
              </td>
      ...
      {pages > 1 && (
        <p>
          ...
          {page > 1 && (
            <>
              <Link href={`/docs/roll-call-votes/${slug}?page=${page - 1}`}>Newer</Link>
              {page < pages ? " · " : ""}
            </>
          )}
          {page < pages && <Link href={`/docs/roll-call-votes/${slug}?page=${page + 1}`}>Older</Link>}
        </p>
      )}
      <hr />
      <p>
