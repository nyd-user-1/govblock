...
export function StreamingResponse({
  ...
  // The sources are not an action. Turning the completion actions off must
  // still leave the reader what the answer was read out of, so the two are
  // gated apart (govblock, 2026-09-09).
  const shouldShowActions =
    showActions && !streaming && (canCopy || onRetry || complete);
  const shouldShowFooter = !streaming && (shouldShowActions || hasSources);
