      ...
      >
        {SORTS.map((s) => (
          <DropdownMenuItem
            ...
            onClick={() =>
              writeUrlParams({ sort: s.value }, { history: "replace" })
            }
          >
            {s.label}
        ...
        <DropdownMenuItem
          className="whitespace-nowrap"
          disabled={!current}
          onClick={() => writeUrlParams({ sort: null }, { history: "replace" })}
        >
          Clear
        </DropdownMenuItem>
      </DropdownMenuContent>
...
export function ShellFooterLinks({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "ml-auto flex shrink-0 items-center gap-4 text-sm",
        className
      )}
    >
...
export function WorkspaceFooter({
  ...
  return (
    <div
      className={cn(
        "flex min-w-0 flex-1 items-center gap-1 lg:gap-2",
        className
      )}
    >
      <Tooltip>
        <TooltipTrigger
          render={
            <Button
              ...
              aria-label={
                panelOpen ? "Hide the customizer" : "Show the customizer"
              }
          ...
          {WORKSPACES.map((w) => {
            ...
            return (
              <DropdownMenuItem
                key={w.key}
                className="whitespace-nowrap"
                onClick={() => w.key !== mode && router.push(w.href)}
              >
                <Icon />
                {w.label}
