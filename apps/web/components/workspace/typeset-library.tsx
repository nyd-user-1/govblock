...
/** The filter box, and the `/` door when what is typed starts with "/". */
function LibraryFilter({ listing, navigate }: { listing: Listing; navigate: ReturnType<typeof useLibraryNavigation> }) {
  const router = useRouter()
...
function WorkTable({ works, flags }: { works: WorkItem[]; flags: boolean }) {
  return (
    <table className="w-full table-fixed text-sm">
      <thead>
        <tr className="border-b text-left text-xs text-muted-foreground">
