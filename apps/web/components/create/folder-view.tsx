  ...
  switch (node.kind) {
    ...
    case "forks":
      return [
        ...
        { key: "title", label: "Copied from", cell: (r) => muted(r.record?.kind === "fork" ? `${stateName(r.record.fork.state)}${r.record.fork.session_id ? ` · ${r.record.fork.session_id}` : ""} — ${truncate(r.record.fork.title ?? "", 90)}` : "") },
