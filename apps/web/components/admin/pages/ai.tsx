...
const days = Array.from({ length: 30 }, (_, i) => {
  ...
  return {
    ...
    gpt: Math.round(base * 100 + Math.random() * 300),
    claude: Math.round(base * 70 + Math.random() * 200),
    gemini: Math.round(base * 30 + Math.random() * 100),
    cost: Math.round((base * 12 + Math.random() * 30) * 100) / 100,
  }
})
const usageConfig: ChartConfig = {
  gpt: { label: "GPT-4o", color: "var(--chart-1)" },
  claude: { label: "Claude 3.5 Sonnet", color: "var(--chart-2)" },
  gemini: { label: "Gemini 1.5 Pro", color: "var(--chart-3)" },
}
const providers = [
  {
    name: "OpenAI",
...
const latency = Array.from({ length: 24 }, (_, i) => ({
  h: `${i}:00`,
  p99: 800 + Math.round(Math.random() * 300),
  p90: 500 + Math.round(Math.random() * 200),
  p50: 250 + Math.round(Math.random() * 100),
}))
