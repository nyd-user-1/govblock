...
export function NewsroomScoped({
  ...
  const { data, isLoading } = usePolicy<Newsroom>(scoped ? "newsroom" : null, {
    state,
  })

  const otherDesks = React.useMemo(
    () => desks.filter((d) => d !== state).slice(0, 3),
    [desks, state]
  )
  ...
  const { data: headlines } = usePolicy<NewsStory[]>(
    scoped ? "stories" : null,
    { state },
    { limit: 8 }
  )
  ...
  if (!data) {
    return (
      <p className="container-wrapper px-6 py-10 text-sm text-muted-foreground">
        {isLoading
          ? `Loading the ${stateName(state)} desk…`
          : `Nothing on file for ${stateName(state)}.`}
      </p>
    )
  }
