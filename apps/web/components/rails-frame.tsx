...
const SHEET = "absolute inset-y-0 z-40 bg-background transition-transform duration-300 ease-out [&_[data-slot=sidebar]]:w-72! [&_[data-slot=sidebar-content]]:flex!"
...
export function RailsFrame({ children }: { children: React.ReactNode }) {
  return (
    <div className="container-wrapper flex flex-1 flex-col overflow-x-clip px-2">
      ...
      >
        <div className={`${SHEET} left-0 [[data-rail-left=closed]_&]:-translate-x-[calc(var(--spacing)*66)]`}>
          <RailStrip side="left" />
          <DocsSidebar />
        </div>
        {/* The strips' 24px kept clear on either side. */}
        <div className="flex min-w-0 flex-1 flex-col px-6">{children}</div>
        <div className={`${SHEET} right-0 [[data-rail-right=closed]_&]:translate-x-[calc(var(--spacing)*66)]`}>
          <RailStrip side="right" />
          <Sidebar
            side="right"
            collapsible="none"
            className="sticky top-[calc(var(--header-height)+0.6rem)] z-30 ml-auto hidden h-[calc(100svh-10rem)] w-72 shrink-0 overflow-visible overscroll-none bg-transparent [--sidebar-menu-width:--spacing(56)] lg:flex"
          >
