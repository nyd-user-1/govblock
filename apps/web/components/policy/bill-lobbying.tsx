...
const lobbyistHref = (name: string) =>
  `/docs/lobbying/lobbyists/${encodeURIComponent(name)}`
const firmHref = (name: string) =>
  `/docs/lobbying/firms/${encodeURIComponent(name)}`
const clientHref = (name: string) =>
  `/docs/lobbying/clients/${encodeURIComponent(name)}`

export function BillLobbyingBlock({
  bill,
  data,
}: {
  bill: string
  data: BillLobbying | null
}) {
  ...
  const years =
    ...
    summary.first_year !== summary.last_year
      ? `${summary.first_year} to ${summary.last_year}`
      : String(summary.last_year ?? "")
  ...
  return (
    <>
      ...
      <p>
        <code>{fmtNumber(summary.clients)}</code>{" "}
        {summary.clients === 1 ? "client" : "clients"} hired{" "}
        <code>{fmtNumber(summary.firms)}</code>{" "}
        {summary.firms === 1 ? "firm" : "firms"} and{" "}
        <code>{fmtNumber(summary.lobbyists)}</code> registered{" "}
        {summary.lobbyists === 1 ? "lobbyist" : "lobbyists"} who named{" "}
        <Chip>{bill}</Chip> in <code>{fmtNumber(summary.filings)}</code>{" "}
        quarterly {summary.filings === 1 ? "filing" : "filings"}
        {years ? `, ${years}` : ""}. Reported under the Lobbying Disclosure Act;
        a filing&rsquo;s income covers everything its registrant worked that
        quarter, so the amounts below are the filings&rsquo;, not this
        bill&rsquo;s.
      </p>
      ...
      {clients.length < summary.clients && (
        <p>
          Who paid to be heard: the <code>{fmtNumber(clients.length)}</code>{" "}
          that filed most often, of <code>{fmtNumber(summary.clients)}</code>.
        </p>
      )}
      ...
      {lobbyists.length < summary.lobbyists && (
        <p>
          Named on the filings that cite the bill: the{" "}
          <code>{fmtNumber(lobbyists.length)}</code> named most often, of{" "}
          <code>{fmtNumber(summary.lobbyists)}</code>.
        </p>
      )}
      ...
      <p>
        The documents themselves, on the Senate&rsquo;s Lobbying Disclosure
        site, largest reported first.
      </p>
              ...
              <tr key={row.filing_uuid}>
                ...
                <td className="pr-8 text-right whitespace-nowrap">
                  {row.document_url || row.url ? (
                    <a
                      href={(row.document_url || row.url) as string}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      {row.filing_type
                        ? truncate(row.filing_type, 22)
                        : "Filing"}
                    </a>
                  ) : (
                    "—"
