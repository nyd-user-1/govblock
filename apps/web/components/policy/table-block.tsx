...
// rows, See more at the bottom left for the rest, copy at the top right on
// hover. No scroller — the list is as tall as it is.
//
...
export function TableBlock({ children, rows }: { children: React.ReactNode; rows: number }) {
  ...
  return (
    <div className="group relative mt-6 flex flex-col gap-2 rounded-2xl bg-surface p-6 text-sm text-surface-foreground">
      ...
      />
      <div ref={ref} data-open={open} className="data-[open=false]:[&_tbody_tr:nth-child(n+11)]:hidden">
        {children}
      </div>
      {hidden > 0 && (
        <div className="flex">
