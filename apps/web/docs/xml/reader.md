...
Against `dc02906`, the front ends the pipeline stores with. Each printing's
...
|---|---|---|---|---|---|
| TX H.B. 18, enrolled, `/us-tx/bill/2025s2/hb/18` | 100% | 11 sections (the bill's and the quoted subchapter's), 12 subsections, 4 paragraphs, 7 subparagraphs; 1 `quotedContent`; `enactingFormula` | nothing | 0 | exact, exact |
| TX H.R. 128, enrolled | 100% | `resolvingClause`, 27 p, 3 `del` (the struck "[or]", "[the]", "[of $500]") | nothing | 0 | exact, exact |
| CA A.B. 1607, enrolled, `/us-ca/bill/2025/ab/1607` | 100% | 4 sections, 11 subsections, 7 paragraphs, 7 subparagraphs; 2 `quotedContent`, one per amending section; `enactingFormula` | nothing | 0 | exact, exact |
| CA A.B. 2052, enrolled | 98% | 4 sections, 22 subsections, 10 paragraphs; 1 `quotedContent`; `longTitle`, `enactingFormula` | nothing | 0 | exact, exact |
| CA S.B. 908, enrolled (`state_link`) | 96% | 48 p: the stored text is leginfo's web page | nothing | 0 | exact, exact |

Front end under 7 ms and `uslmToDoc` under 9 ms on each. Both enacting
formulas read whole ("BE IT ENACTED BY THE LEGISLATURE OF THE STATE OF
TEXAS:", "The people of the State of California do enact as follows:"), and
every amending section holds the law it amends as `quotedContent`.

Left on these printings: H.B. 18 splits its instruction at the first
"Subchapter G", so "Subchapter G to read as follows:" opens the quotation and
the quoted subchapter is a paragraph, not a `subchapter`; section 301.101's
heading carries its first words ("DEFINITIONS. In this subchapter:").
A.B. 2052's "( l )", a spaced letter, is the one unmatched enumerator.
