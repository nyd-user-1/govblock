...
- **Second rebuild batch, `rebuild-w8-c51724f`**, 317 jobs: Kansas, Nevada, Maryland, Louisiana and South Carolina statutes; Utah, Kentucky, New Hampshire, New Mexico and Vermont bills. The box was pulled fast-forward to c51724f. The controller bundles the front ends when it starts, so it had to be restarted, and the first restart went wrong: the `pgrep` pattern that found the old controller also matched the ssh shell running it, and `kill` took both. Only window 8's own controller and shell were stopped; window 4's Virginia re-fetch (pid 3340) ran on untouched. The controller was then started again, checked with `ps -C node`, logging to `logs/watch-window8-2.log`.
