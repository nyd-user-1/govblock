...
export const API_RESOURCES: ApiResource[] = [
  ...
  },
  {
    ...
    options: [...SCOPE, ["id <bill_id>", "the bill's id"], ["document <id>", "one version, by document id"], ["format raw", "plain text instead of JSON"]],
    example: { id: 2057965 },
  },
  {
    name: "members",
    ...
    options: [...SCOPE, ["chamber <name>", "House, Senate or Assembly"], ["party <letter>", "D, R or I"]],
  },
  {
    name: "member",
  ...
  {
    name: "record",
    summary: "a member's record in a session: sponsored, co-sponsored, aye and nay",
    lead: "What a member put their name to and how they voted, a page of each, with the counts.",
    options: [...SCOPE, ["id <people_id>", "the member's id"], ["limit <n>", "rows per list"], ["offset <n>", "rows to skip"]],
  ...
  },
  {
    ...
    options: [...SCOPE, ["committee <name>", "the committee, as the record spells it"], ...PAGE],
  ...
  },
  {
    ...
    options: [["state <code>", "the jurisdiction"], ["titles 1", "carry each session's own title"]],
    example: { titles: 1 },
  },
  {
    name: "search",
    ...
    options: [...SCOPE, ["q <text>", "at least two characters"], ["all 1", "every jurisdiction, the one in scope first"], ["text 1", "search the bills' text as well"], ["limit <n>", "rows per section (max 20)"]],
    example: { q: "water", limit: 5 },
  },
  {
    name: "actions",
    summary: "a bill's actions as congress.gov files them, with codes and committees",
  ...
  },
  {
    ...
    options: [["member <people_id>", "the member's id"], ["bioguide <id>", "or their bioguide id"]],
...
export const DATASET_FAMILIES: { name: string; summary: string }[] = [
  { name: "bills", summary: "every bill, with status, latest action, committee and sponsor" },
  { name: "sponsors", summary: "every sponsorship: bill, member, position" },
  { name: "members", summary: "everyone who sat, with party, chamber, district" },
